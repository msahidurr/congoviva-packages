...
import { Save, CreditCard, AlertCircle, Banknote, IndianRupee, Wallet, Coins, Search, X, HandCoins } from 'lucide-react';
...
export default function PaymentSettings({ settings = {} }: PaymentSettingsProps) {
  ...
  const paymentMethods = useMemo(() => [
    { key: 'manual', name: t(PAYMENT_METHOD_LABELS[PAYMENT_METHODS.MANUAL]) },
        ...
        <div className="space-y-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-6">
              ...
              {/* Manual Payment */}
              {shouldShowMethod('manual') && (
              <PaymentMethodCard
                title={t(PAYMENT_METHOD_LABELS[PAYMENT_METHODS.MANUAL])}
                icon={<HandCoins className="h-5 w-5" />}
                enabled={data.is_manually_enabled}
                onToggle={(checked) => setData('is_manually_enabled', checked)}
              >

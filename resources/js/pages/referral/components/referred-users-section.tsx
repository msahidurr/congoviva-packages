    ...
    <div className="space-y-6">
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="mt-2 text-2xl font-bold">{resolvedCurrencySymbol}{getTotalCommissionAll().toFixed(2)}</p>
              </div>
                    ...
                    <div className="flex items-start space-x-6 flex-shrink-0">
                      <div className="text-right">
                        {(() => {
                          ...
                          return planInfo ? (
                            <div>
                              <Badge variant="default" className="mb-1.5 rounded-full">
                                {planInfo.name}
                              </Badge>
                              <p className="text-sm text-muted-foreground">
                                {resolvedCurrencySymbol}{planInfo.price}/{t(planInfo.cycle)}
                              </p>
                            </div>
                          ) : (
                      ...
                      {getTotalCommission(user) > 0 && (
                        <div className="text-right min-w-[80px]">
                          <p className="text-sm font-semibold text-green-600">
                            +{resolvedCurrencySymbol}{getTotalCommission(user)}
                          </p>
                        ...
                        {user.referrals.map((referral) => (
                          <div key={referral.id} className="flex justify-between items-center">
                            ...
                            <span className="text-sm font-semibold text-green-600">
                              +{resolvedCurrencySymbol}{referral.amount}
                            </span>
                          </div>
                        ))}
                      </div>
      ...
      {referredUsers.last_page > 1 && (
        <Pagination
          ...
          entityName={t('users')}
          onPageChange={(url) => {
            router.visit(`${url}#referred-users`, {
              preserveState: true,
              preserveScroll: true,
              only: ['referredUsers']
            });

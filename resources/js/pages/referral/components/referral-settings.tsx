    ...
    post(route('referral.settings.update'), {
      ...
      onError: (formErrors) => {
        ...
        if (typeof formErrors === 'string') {
          ...
          toast.error(t('Failed to update referral settings: {{errors}}', { errors: Object.values(formErrors).join(', ') }));
        }
      }
    });
  };
      ...
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <Label htmlFor="threshold_amount">{t('Minimum Threshold Amount')} {resolvedCurrencySymbol}</Label>
              <Input
                id="threshold_amount"
                type="number"
                step="0.01"

...
export function CrudFormModal({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    formConfig.fields.forEach(field => {
      ...
      const isRequired = typeof field.required === 'function' ? field.required(mode, processedData) : field.required;
      
      // Check if field is conditionally required based on other field values
      const isConditionallyRequired = field.conditional ? field.conditional(mode, processedData) : true;
      
      if (isRequired && isConditionallyRequired && !processedData[field.name]) {
  ...
  const renderField = (field: FormField) => {
    ...
    // Evaluate required if it's a function
    const isRequired = typeof field.required === 'function' ? field.required(mode, formData) : field.required;
    ...
    switch (field.type) {
        ...
        return (
          <div className="relative">
            <Input
              ...
              onChange={(e) => handleChange(field.name, e.target.value)}
              required={isRequired}
              className={errors[field.name] ? 'border-red-500 pr-10' : 'pr-10'}
              disabled={mode === 'view'}
            />
            <button
              type="button"
      ...
      case 'select':
        ...
        return (
          <Select
            ...
            required={isRequired}
          >
            <SelectTrigger className={errors[field.name] ? 'border-red-500' : ''} required={isRequired}>
      ...
      case 'media-picker':
        // Get image URL from formData or initialData
        let currentImageUrl = formData[field.name] || '';
        
        // If in edit mode and no new image selected yet, use the initialData image
        if (!currentImageUrl && mode === 'edit' && initialData[field.name]) {
        ...
        return (
          <MediaPicker
            value={getImageDisplayUrl(currentImageUrl)}
  ...
  const shouldRenderFieldLabel = (field: FormField) => {
            ...
            {formConfig.priceSummary && (
              <div className="bg-gray-50 p-4 rounded-lg mb-4">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className="font-medium">{window?.appSettings?.currencySettings?.currencySymbol || '$'}{formConfig.priceSummary.unitPrice.toFixed(2)}</span>
                </div>
                <div className="flex justify-between items-center mb-2">
              ...
              <div style={{ display: 'grid', gridTemplateColumns: `repeat(${columns}, 1fr)`, gap: '1rem' }}>
                {formConfig.fields.map((field) => {
                  ...
                  return (
                    ...
                    >
                      {shouldRenderFieldLabel(field) && (
                        <Label htmlFor={field.name} className="text-sm font-medium" required={typeof field.required === 'function' ? field.required(mode, formData) : field.required}>
                          {field.label}
                        </Label>
                      )}
                      {renderField(field)}

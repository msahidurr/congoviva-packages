...
export function PageCrudWrapper({ 
  ...
  const defaultEntityLabel = entity.name.endsWith('ies')
    ? `${entity.name.slice(0, -3)}y`
    : entity.name.endsWith('s')
      ? entity.name.slice(0, -1)
      : entity.name;
  const resolvedEntityLabel = entityLabel || (defaultEntityLabel.charAt(0).toUpperCase() + defaultEntityLabel.slice(1));
  const defaultAddButtonLabel = `Add New ${resolvedEntityLabel}`;
  ...
  if (showAddButton && hasPermission(permissions, entity.permissions.create)) {
    pageActions.push({
      label: addButtonLabel || defaultAddButtonLabel,
    ...
    >
      ...
      {formMode === 'view' && ViewComponent ? (
        <Dialog open={isFormModalOpen} onOpenChange={(open) => { if (!open) setIsFormModalOpen(false); }}>
          <ViewComponent {...{[entity.name.slice(0, -1)]: viewData}} />
        </Dialog>
      ) : (
        <CrudFormModal
          ...
          title={
            formMode === 'create' 
              ...
              : formMode === 'edit' 
                ? `Edit ${resolvedEntityLabel}` 
                : `View ${resolvedEntityLabel}`
          }
          mode={formMode}
      ...
      <CrudDeleteModal
        ...
        entityName={resolvedEntityLabel.toLowerCase()}
      />
    </PageTemplate>
  );
}

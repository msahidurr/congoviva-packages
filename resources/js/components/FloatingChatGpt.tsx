...
export function FloatingChatGpt() {
  ...
  if (isSuperAdmin) {
    ...
    canUseChatGPT = hasActivePlan && auth?.user?.plan?.features?.ai_integration === true;
  } else {
    ...
    canUseChatGPT = hasActivePlan && creator?.plan?.features?.ai_integration === true;
  }
  ...
  return createPortal(
    <>
      <div 
        className="fixed bottom-6 rtl:left-6 ltr:right-6 z-[80000] pointer-events-auto"
        data-chatgpt-button

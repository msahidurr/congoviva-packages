...
export function CrudTable({
  ...
  const hasVisibleActions = data.some((row) =>
    actions.some((action) => {
      ...
      if (action.condition && !action.condition(row, context)) {
        return false;
      }
    ...
    return (
      <div className="flex items-center justify-end space-x-2">
        {actions.map((action, index) => {
          ...
          // Handle dynamic icons (functions) and static icons (strings)
      ...
      <Table>
        ...
        <TableBody>
          {data.length > 0 ? (
            ...
            <TableRow>
              <TableCell 
                colSpan={columns.length + (showIndexColumn ? 1 : 0) + (hasVisibleActions ? 1 : 0)} 

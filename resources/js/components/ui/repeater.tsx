        ...
        >
          ...
          <div className="flex items-center justify-between">
            ...
            {items.length > minItems && (
              ...
              >
                <Trash2 className={cn('h-4 w-4', removeButtonText ? 'mr-1' : '')} />
                {removeButtonText ? removeButtonText : null}
              </Button>
            )}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {fields.map((field) => (
              <div key={field.name} className={cn('space-y-2', field.colSpan === 2 && 'col-span-2')}>
                <Label htmlFor={`${field.name}_${index}`} className="text-sm font-medium dark:text-gray-200" required={field.required}>
                  {field.label}
                </Label>
                {renderField(
                  field,

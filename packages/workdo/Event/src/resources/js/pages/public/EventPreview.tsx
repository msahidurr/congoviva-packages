...
const syncPreviewProps = (previewData: any) => {
  if (!previewData?.base_url && !previewData?.image_url) {
    return;
  }
  ...
  if (previewData.base_url) {
    (window as any).page.props.base_url = previewData.base_url;
  }

  if (previewData.image_url) {
    (window as any).page.props.image_url = previewData.image_url;
  }
...
export default function EventPreviewPage() {
  ...
  useEffect(() => {
    const data = localStorage.getItem('event_qr_preview_data');
  ...
  return (
    <>
      <Head>
        <title>{previewData.name || t('Event Preview')}</title>
      </Head>

      <EventPreview
        data={{
          name: previewData.name || t('Preview Event'),
          event_type: previewData.event_type || 'conference',

...
import { Calendar, Star, Users, Zap, Shield, Globe, Heart, Share, BarChart3, Smartphone, Eye, QrCode } from 'lucide-react';
...
export default function EventMarketplace({ marketplaceData }: Props) {
  const { t } = useTranslation();
  const brandColor = marketplaceData.primary_color || '#10b77f';
    ...
    switch (sectionKey) {
      case 'hero':
        const heroStyle = marketplaceData.use_background_image && marketplaceData.background_image
          ...
          : {
              background: `linear-gradient(135deg, ${brandColor} 0%, ${marketplaceData.secondary_color || '#3b82f6'} 50%, ${marketplaceData.accent_color || '#8b5cf6'} 100%)`
            };
                ...
                <svg className="w-full h-full" viewBox="0 0 400 400" xmlns="http://www.w3.org/2000/svg">
                  <defs>
                    <pattern id="eventPattern" x="0" y="0" width="80" height="80" patternUnits="userSpaceOnUse">
                      <rect x="32" y="32" width="16" height="16" rx="2" fill="none" stroke="currentColor" strokeWidth="2" opacity="0.3"/>
                  ...
                  <rect width="100%" height="100%" fill="url(#eventPattern)" className="text-white"/>
                </svg>
              </div>
            )}
              ...
              <div className="text-center mb-12">
                ...
                <div className="flex flex-col sm:flex-row gap-4 justify-center mb-12">
                  <Button 
                    ...
                    onClick={() => window.open(marketplaceData.demo_button_link || route('event-qr-code.preview'), '_blank')}
                  >
          ...
          <section key="features" className="py-12 sm:py-16 lg:py-20" style={{ backgroundColor: marketplaceData.features_background_color || '#ffffff' }}>
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              <div className="text-center mb-12">
                ...
                <p className="text-xl text-gray-600">{marketplaceData.features_subtitle || t('Everything you need for professional event QR code management')}</p>
              </div>
              ...
              <div className={`${marketplaceData.features_layout === 'list' ? 'space-y-6' : marketplaceData.features_layout === 'carousel' ? 'flex overflow-x-auto gap-6 pb-4' : marketplaceData.features_layout === 'centered' ? 'flex flex-col items-center space-y-8 max-w-2xl mx-auto' : `grid grid-cols-1 ${(marketplaceData.features_columns || 4) >= 2 ? 'sm:grid-cols-2' : ''} ${(marketplaceData.features_columns || 4) >= 3 ? 'lg:grid-cols-3' : ''} ${(marketplaceData.features_columns || 4) >= 4 ? 'xl:grid-cols-4' : ''} gap-6 sm:gap-8`}`}>
                {(marketplaceData.features || []).map((feature, index) => {
                  ...
                  const iconColor = marketplaceData.features_icon_color || '#10b77f';
                  return (
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              <div className="text-center mb-8 sm:mb-12 lg:mb-16">
                <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-4">
                  {marketplaceData.screenshots_title || t('See Event QR Code in Action')}
                </h2>
                <p className="text-lg text-gray-600 max-w-3xl mx-auto leading-relaxed font-medium">
                  {marketplaceData.screenshots_subtitle || t('Discover how easy it is to create and manage event QR codes with our intuitive interface and powerful features.')}
                </p>
              </div>
